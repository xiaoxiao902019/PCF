...
for i in range(len(data)):
    fig,ax = plt.subplots()
    indices = [n for n, x in enumerate(data[i][2]) if x == min(data[i][2])]
    start = indices[len(indices)-1]
    color = ['k','b','c']   
    U_0 = np.average(data[i][2][:indices[0]])
    print(U_0)
    for n in range(start,len(data[i][2])):
        if data[i][2][n] > U_0-0.5 :
            end = n
            break 
    y = 1/c_I2(data[i][2][start+100:end],U_0)
    print(c_I2(data[i][2][start+100:end],U_0))
    x = np.array(data[i][1][start+100:end])-data[i][1][start+100]
#    if i%3 == 0:
#        fig = ax1
#    if i%3 == 1:
#        fig = ax2
#    if i%3 == 2:
#        fig = ax3
    plt.plot(x,y,".",ms = 0.5,fillstyle = 'full',mec = color[i//3],mfc =color[i//3])
